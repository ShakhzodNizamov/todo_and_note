...
import uz.jagito.todoandnote.data.HAVE_TIME
...
class AllTodoRecyclerAdapter :
    RecyclerView.Adapter<AllTodoRecyclerAdapter.ViewHolder>(), BaseAdapter {
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        private fun loadMenu(view: View) {
            ...
            menu.menuInflater.inflate(R.menu.all_todo_popup_menu, menu.menu)

            if(pos != HAVE_TIME){
                menu.menu.findItem(R.id.actionDoneAllTodo).isVisible = false
                menu.menu.findItem(R.id.actionEditAllTodo).isVisible = false
                menu.menu.findItem(R.id.actionCancelAllTodo).isVisible = false
            }

            menu.setOnMenuItemClickListener { menuItem ->
                when (menuItem.itemId) {
                    R.id.actionCloneAllTodo -> {
                        listenerClone?.invoke(sortedList[adapterPosition])
                    }
                    R.id.actionDeleteAllTodo -> {
                        listenerDelete?.invoke(sortedList[adapterPosition])
                    }
                    R.id.actionEditAllTodo -> {
                        listenerEdit?.invoke(sortedList[adapterPosition])
                    }
                    R.id.actionCancelAllTodo -> {
                        listenerCancelled?.invoke(sortedList[adapterPosition])
                    }
                    R.id.actionDoneAllTodo -> {
                        listenerDone?.invoke(sortedList[adapterPosition])
                    }
                }
                true
            }
