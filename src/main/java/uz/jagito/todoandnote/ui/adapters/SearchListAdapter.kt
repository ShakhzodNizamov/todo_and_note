...
class SearchListAdapter :
    RecyclerView.Adapter<SearchListAdapter.ViewHolder>(), BaseAdapter, Filterable {
    private val dataForFilter = ArrayList<TodoData>()
        ...
        private fun loadMenu(view: View) {
            ...
            menu.setOnMenuItemClickListener { menuItem ->
                when (menuItem.itemId) {
                    ...
                    R.id.action_cancel -> {
                        listenerCancel?.invoke(sortedList[adapterPosition])
                    }
                }
                true
            }
    ...
    override fun delete(it: TodoData) {
        ...
        dataForFilter.remove(it)
    }

    override fun add(it: TodoData) {
        sortedList.add(it)
        dataForFilter.add(it)
    }
    ...
    private val todoFilter: Filter = object : Filter() {
        override fun performFiltering(constraint: CharSequence): FilterResults {
            ...
            if (constraint.isEmpty()) {
                ...
                for (i in dataForFilter) {
                    if (i.title.contains(filterPattern) ||
                        i.hashTag.contains(filterPattern)
                    ) {
                        filteredList.add(i)
                    }
                }
        ...
        override fun publishResults(
            ...
            sortedList.clear()
            sortedList.addAll(results.values as Collection<TodoData>)
            notifyDataSetChanged()
        }
    }

    override fun getFilter(): Filter = todoFilter
}
